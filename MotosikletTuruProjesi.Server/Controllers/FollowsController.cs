...
namespace MotosikletTuruProjesi.Controllers
{
    ...
    public class FollowsController : ControllerBase
    {
        ...
        public async Task<IActionResult> ToggleFollow([FromBody] UserFollow follow)
        {
            ...
            if (existing != null) { _context.UserFollows.Remove(existing); await _context.SaveChangesAsync(); return Ok(new { status = "unfollowed" }); }

            _context.UserFollows.Add(follow); await _context.SaveChangesAsync(); return Ok(new { status = "followed" });
        }
        ...
        // "Şu an takip ediyor muyum?" kontrolü
        [HttpGet("check")]
        public IActionResult CheckFollow(string follower, string followed)
        {
            // Veritabanında bu ikili var mı diye bakıyoruz
